...
fun main() {
    ...
    fun calculateStep(from: Int, to: Int): Int {
        var counter = 0
        var i = abs(from - to)
        while (i > 0) {
            counter += i
    ...
    val testInput = readInput("Day07_test")[0].split(",").map { s -> s.toInt() }.sorted()
    check(part1(testInput) == 37)
    check(part2(testInput) == 168)

    val input = readInput("Day07")[0].split(",").map { s -> s.toInt() }.sorted()
    println(part1(input))
    println(part2(input))
}
